...
export default function Navbar() {
  ...
  return (
    <>
      {/* Desktop top nav */}
      <header className="hidden md:flex fixed top-0 left-0 right-0 z-40 h-14 bg-surface border-b border-border">
        <div className="max-w-5xl mx-auto w-full px-6 flex items-center justify-between">
          <Link href="/" className="font-semibold text-primary tracking-tight">
            كلمات <span className="text-muted font-normal text-sm ml-1">Kalimat</span>
          </Link>
        ...
        <div className="flex items-stretch h-16">
          {links.map((link) => {
            ...
            return (
              ...
              >
                <span className={cn('p-1.5 rounded-lg', active && 'bg-primary/10')}>
                  {link.icon}
                </span>
                {link.label}
              </Link>
