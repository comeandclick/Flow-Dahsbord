...
function isStoreBootstrapAllowed() {
  return process.env.NODE_ENV !== "production";
}

function createStoreUnavailableError(action, status = 503) {
  const error = new Error(
    `Le stockage des comptes est indisponible pour le moment (${action}).`,
  );
  error.status = status;
  error.code = "FLOW_STORE_UNAVAILABLE";
  return error;
}
...
async function createJsonBlobStore(store) {
  if (!isStoreBootstrapAllowed()) {
    throw createStoreUnavailableError("création du store", 503);
  }
...
export async function readStore() {
  ...
  if (!response.ok) {
    if (response.status === 404) {
      ...
      throw createStoreUnavailableError("lecture du store", 503);
    }
    throw createStoreUnavailableError(`lecture du store (${response.status})`, 503);
  }
...
export async function writeStore(store) {
  ...
  if (response.status === 404) {
    if (!isStoreBootstrapAllowed()) {
      throw createStoreUnavailableError("écriture du store", 503);
    }
  ...
  if (!response.ok) {
    throw createStoreUnavailableError(`écriture du store (${response.status})`, 503);
  }
}
