...
export async function getSubscription(userId) {
  ...
  try {
    ...
    const account = store.users.find((entry) => entry.uid === userId);

    if (!account?.db?.subscription?.stripeCustomerId) {
      return null;
    }

    const subscriptions = await stripe.subscriptions.list({
      customer: account.db.subscription.stripeCustomerId,
      status: "all",
      limit: 1,
    });
    ...
    return {
      ...
      planName: PLAN_CATALOG[plan]?.name || "Plan inconnu",
