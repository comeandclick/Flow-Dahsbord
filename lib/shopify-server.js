import { readSessionCookieValue } from "./auth.js";
import { readStore } from "./remote-store.js";
...
async function resolveUserShopifyConfig(req) {
  const session = readSessionCookieValue(readCookieValue(req, "flow_session"));
  if (!session?.uid) {
    return { session: null, storeDomain: "", accessToken: "" };
  }
  ...
  return {
    session,
    storeDomain: normalizeStoreDomain(config.storeDomain),
    accessToken: trim(config.accessToken),
  };
}
...
export async function shopifyProxyHandler(req, res) {
  ...
  try {
    const userConfig = await resolveUserShopifyConfig(req);
    const storeDomain = userConfig.storeDomain;
    ...
    if (endpointRaw === "__status") {
      res.status(200).json({
        ...
        error: !storeDomain || !accessToken ? "Aucune boutique Shopify n'est connectee sur ce compte." : "",
        storeDomain,
      });
      return;
    }
    ...
    if (!storeDomain) {
      res.status(200).json({ ready: false, error: "Aucune boutique Shopify n'est connectee sur ce compte." });
      return;
    }
    if (!accessToken) {
      res.status(200).json({ ready: false, error: "Aucune boutique Shopify n'est connectee sur ce compte." });
