...
function clampStringList(list, accepted, fallback) {
  if (!Array.isArray(list) || !list.length) return fallback;
  const unique = [];
  list.forEach((item) => {
    if (!accepted.includes(item) || unique.includes(item)) return;
    unique.push(item);
  });
  accepted.forEach((item) => {
    if (!unique.includes(item)) unique.push(item);
  });
  return unique.slice(0, accepted.length);
}
...
export function normalizeDb(input, user) {
  ...
  return {
    ...
    settings: {
      ...
      dashboardLayout: clampEnum(db.settings?.dashboardLayout, ["overview", "immersive"], base.settings.dashboardLayout),
      sidebarLocked: Boolean(db.settings?.sidebarLocked),
      dashboardArrangement: {
        metrics: clampStringList(
          db.settings?.dashboardArrangement?.metrics,
          base.settings.dashboardArrangement.metrics,
          base.settings.dashboardArrangement.metrics,
        ),
        focus: clampStringList(
          db.settings?.dashboardArrangement?.focus,
          base.settings.dashboardArrangement.focus,
          base.settings.dashboardArrangement.focus,
        ),
        mini: clampStringList(
          db.settings?.dashboardArrangement?.mini,
          base.settings.dashboardArrangement.mini,
          base.settings.dashboardArrangement.mini,
        ),
      },
      shopify: {
        storeDomain: clampText(db.settings?.shopify?.storeDomain || "", 160),
        accessToken: clampText(db.settings?.shopify?.accessToken || "", 2000),
        connectedAt: clampText(db.settings?.shopify?.connectedAt || "", 40),
        validatedAt: clampText(db.settings?.shopify?.validatedAt || "", 40),
      },
      demoFixtures: {
        seededAt: clampText(db.settings?.demoFixtures?.seededAt || "", 40),
        shopifyOrders: clampArray(db.settings?.demoFixtures?.shopifyOrders, 80).map((item) => ({
          id: clampText(item?.id, 80),
          name: clampText(item?.name, 80),
          order_number: clampText(item?.order_number, 40),
          created_at: clampText(item?.created_at, 40),
          cancelled_at: clampText(item?.cancelled_at, 40),
          cancel_reason: clampText(item?.cancel_reason, 160),
          total_price: `${Number(item?.total_price) || 0}`,
          financial_status: clampText(item?.financial_status || "", 40),
          fulfillment_status: clampText(item?.fulfillment_status || "", 40),
          email: clampText(item?.email || "", 180),
          customer: {
            first_name: clampText(item?.customer?.first_name || "", 80),
            last_name: clampText(item?.customer?.last_name || "", 80),
          },
          line_items: clampArray(item?.line_items, 24).map((lineItem) => ({
            product_id: clampText(lineItem?.product_id, 80),
            variant_id: clampText(lineItem?.variant_id, 80),
            title: clampText(lineItem?.title, 180),
            quantity: Math.max(0, Math.min(9999, Number(lineItem?.quantity) || 0)),
            price: `${Number(lineItem?.price) || 0}`,
          })),
        })),
      },
      customBackgrounds: {
        dark: clampText(db.settings?.customBackgrounds?.dark || "", 2_000_000),
        light: clampText(db.settings?.customBackgrounds?.light || "", 2_000_000),
      },
